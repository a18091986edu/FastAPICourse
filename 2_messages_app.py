...
async def get_notes_by_id(note_id: Annotated[int, Path()]) -> dict:
    try:
...
async def update_quote(
    quote_id: Annotated[int, Path()], quote: Annotated[str, Body(...)]
) -> str:
...
async def update_comment(
    comment_id: Annotated[int, Path], comment: Annotated[str, Body(...)]
) -> str:
    if await check_comment(comment_id):
