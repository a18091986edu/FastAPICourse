...
async def delete_note(note_id: Annotated[int, Path()]) -> Note:
    note_index = next(
        (idx for idx, note in enumerate(notes) if note.id == note_id), None
    )
...
@app.patch("/users/{user_id}", status_code=status.HTTP_200_OK, response_model=User)
async def update_user(
    user_id=Annotated[int, Path(...)], user_data=Annotated[UserUpdate, Body(...)]
) -> User:
...
class Task(BaseModel):
    ...
    description: str | None = Field(
        default=None, max_length=500, description="Описание задачи"
    )
...
class Post(BaseModel):
    ...
    created_at: datetime = Field(
        default_factory=datetime.now, description="Запись создана"
    )
...
class User(BaseModel):
    ...
    is_staff: bool = Field(
        default=False, description="Является служебным пользователем"
    )
    is_superuser: bool = Field(default=False, description="Является суперпользователем")
    date_joined: datetime = Field(
        default_factory=datetime.now, description="Зарегистрирован"
    )
